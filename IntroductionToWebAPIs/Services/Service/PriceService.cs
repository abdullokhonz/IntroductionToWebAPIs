...
namespace IntroductionToWebAPIs.Services.Service
{
    public class PriceService : IPriceService
    {
        ...
        public async Task<ServiceResponse<IEnumerable<Price>>> GetAllAsync(CancellationToken ct = default)
        {
            var result = await _repository.GetAllAsync(ct);

            return ServiceResponse<IEnumerable<Price>>.Ok(result, "Items retrieved");
        }

        public async Task<ServiceResponse<Price?>> GetByIdAsync(Guid id, CancellationToken ct = default)
        {
            var existing = await _repository.GetByIdAsync(id, ct);

            if (existing == null)
                return ServiceResponse<Price?>.Fail("Item not found");

            return ServiceResponse<Price?>.Ok(existing, "Item retrieved");
        }

        public async Task<ServiceResponse<Price>> CreateAsync(Price item, CancellationToken ct = default)
        {
            if (item == null)
                return ServiceResponse<Price>.Fail("Item is null");

            var result = await _repository.CreateAsync(item, ct);

            if (result == null)
                return ServiceResponse<Price>.Fail("Failed to create item");

            return ServiceResponse<Price>.Ok(result, "Item created successfully");
        }

        public async Task<ServiceResponse<bool>> UpdateAsync(Guid id, Price item, CancellationToken ct = default)
        {
            if (item == null)
        ...
        public async Task<Price?> PremiumGetByIdAsync(Guid id, CancellationToken ct = default)
        {
